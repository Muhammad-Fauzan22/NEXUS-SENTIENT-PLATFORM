...
import { extractSkillsV2, computeVelocity, normalizeSkill } from '../src/lib/server/analytics/trends_analyzer';
...
async function analyze() {
  ...
  const files = fs.existsSync(JOB_DESC_DIR)
    ? fs.readdirSync(JOB_DESC_DIR).map((f) => path.join(JOB_DESC_DIR, f)).filter((p) => p.endsWith('.csv') || p.endsWith('.json') || p.endsWith('.txt'))
    : [];
  ...
  const entries = Array.from(counts30.entries()).sort((a, b) => b[1] - a[1]).slice(0, 300);
