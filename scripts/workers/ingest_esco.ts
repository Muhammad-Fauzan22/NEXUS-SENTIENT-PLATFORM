...
async function main() {
  ...
  if (fs.existsSync(skillsCsv)) {
    ...
    for (const r of rows) {
      const [code, name] = r;
      const { error } = await supabaseAdmin.from('esco_skills').upsert({ code, name }, { onConflict: 'code' });
  ...
  if (fs.existsSync(occCsv)) {
    ...
    for (const r of rows) {
      const [code, name] = r;
      const { error } = await supabaseAdmin.from('esco_occupations').upsert({ code, name }, { onConflict: 'code' });
