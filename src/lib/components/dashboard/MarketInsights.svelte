<script lang="ts">
	...
	let topDemand: { skill_name: string; mention_count_7d: number; velocity_score: number }[] =
		$state([]);
					...
					{#each topDemand as s}
						<li class="flex items-center justify-between bg-secondary/30 rounded px-3 py-2">
							...
							<a
								...
								target="_blank"
								href={`https://www.google.com/search?q=${encodeURIComponent(s.skill_name + ' course')}`}
								>Find courses</a
							>
						</li>
					{/each}
				...
				<ul class="space-y-1">
					{#each articles as a}
						<li class="bg-secondary/30 rounded px-3 py-2">
							...
							{#if a.published_at}<div class="text-xs text-foreground/70">
