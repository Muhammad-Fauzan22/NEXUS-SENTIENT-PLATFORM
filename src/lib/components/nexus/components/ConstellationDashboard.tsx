...
export const ConstellationDashboard: React.FC<ConstellationProps> = ({
  ...
  const moons = activities.map((activity, index) => {
    ...
    <ConstellationContainer ref={containerRef}>
      ...
      <ConstellationCanvas>
        ...
        <Tooltip title="Analisis Potensi Inti" arrow>
          <CentralStar 
            brightness={1.2}
            onClick={() => handleElementClick('central-star')}
          >
        ...
        {moons.map(moon => (
          ...
          >
            <Moon
              ...
              isVisible={hoveredElement === moon.parentPlanet || selectedElement === moon.parentPlanet}
