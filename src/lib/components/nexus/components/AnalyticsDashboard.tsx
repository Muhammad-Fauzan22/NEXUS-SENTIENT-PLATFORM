...
export const AnalyticsDashboard: React.FC<AnalyticsDashboardProps> = ({ onMetricClick }) => {
	const theme = useTheme();
	const [menuAnchor, setMenuAnchor] = useState<null | HTMLElement>(null);
	const [expandedSections, setExpandedSections] = useState<Set<string>>(
		new Set(['metrics', 'charts'])
	);
		...
		<GlassDashboard elevation={0}>
			...
			<Box>
				<Button
					...
					startIcon={
						expandedSections.has('charts') ? <ExpandLessOutlinedIcon /> : <ExpandMoreOutlinedIcon />
					}
					sx={{
						mb: 2,
						textTransform: 'none',
