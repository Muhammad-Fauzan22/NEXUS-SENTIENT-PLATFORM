...
const ActivityChip = styled(Chip)<{ completed: boolean }>(({ theme, completed }) => ({
	...
	backgroundColor: completed ? `${theme.palette.success.main}30` : `${theme.palette.grey[700]}30`,
	...
	'&:hover': {
		backgroundColor: completed ? `${theme.palette.success.main}50` : `${theme.palette.grey[600]}50`
	}
}));
...
export const LivingDashboard: React.FC<LivingDashboardProps> = ({
	...
	useEffect(() => {
		const notificationTimer = setInterval(() => {
			...
			const randomMessage =
				opportunityMessages[Math.floor(Math.random() * opportunityMessages.length)];
	...
	const formatPillarName = (pillar: DevelopmentPillar): string => {
		switch (pillar) {
			case DevelopmentPillar.ACADEMIC:
				return 'Akademik';
			case DevelopmentPillar.MANAGERIAL:
				return 'Manajerial';
			case DevelopmentPillar.LEADERSHIP:
				return 'Kepemimpinan';
			case DevelopmentPillar.TECHNICAL:
				return 'Teknis';
			case DevelopmentPillar.SOCIAL:
				return 'Sosial';
			default:
				return pillar;
		}
	...
	return (
		<DashboardContainer>
								...
								<Stack direction="row" flexWrap="wrap" gap={1}>
									{pillarActivities.map((activity) => (
										<ActivityChip
											...
											icon={
												userProgress.completedActivities.includes(activity.id) ? (
			...
			>
				<NotificationAlert onClose={handleCloseNotification} severity="info" variant="filled">
