...
import { Box, Typography, ToggleButtonGroup, ToggleButton, Stack, Card, CardContent, Switch, FormControlLabel } from '@mui/material';
...
const XRViewport = styled(Box)<{ mode: XRMode }>(({ theme, mode }) => {
  const getModeColor = () => {
    switch (mode) {
      case XRMode.AR_OVERLAY: return theme.palette.info.main;
      case XRMode.VR_IMMERSIVE: return theme.palette.error.main;
      case XRMode.MR_COLLABORATIVE: return theme.palette.success.main;
...
export const XRInterface: React.FC<XRInterfaceProps> = ({
  ...
  const getModeLabel = (xrMode: XRMode): string => {
    switch (xrMode) {
      case XRMode.AR_OVERLAY: return '🔍 AR Overlay';
      case XRMode.VR_IMMERSIVE: return '🥽 VR Immersive';
      case XRMode.MR_COLLABORATIVE: return '🤝 MR Collaborative';
      ...
      <CardContent>
        <Stack spacing={3}>
          ...
          <XRViewport mode={mode}>
            ...
            {isScanning && (
              <Typography variant="caption" sx={{ position: 'absolute', bottom: 16, color: 'warning.main' }}>
