...
const ModeButton = styled(Button)<{ active: boolean; mode: InteractionMode }>(({
	theme,
	active,
	mode
}) => {
	const getModeColor = () => {
		switch (mode) {
			...
			case InteractionMode.EYE_TRACKING:
				return theme.palette.warning.main;
			case InteractionMode.BIOMETRIC:
				return theme.palette.error.main;
			case InteractionMode.HAPTIC:
...
const CalibrationIndicator = styled(Box)<{ value: number; color: string }>(
	({ theme, value, color }) => ({
		display: 'flex',
		alignItems: 'center',
		gap: theme.spacing(1),
						...
						<Box>
							...
							<CalibrationIndicator value={gestureCalibration} color="#50C878">
								<Typography variant="caption">
									Calibration: {(gestureCalibration * 100).toFixed(1)}%
								</Typography>
					...
					{activeMode === InteractionMode.VOICE && (
						<Box>
							...
							<CalibrationIndicator value={voiceRecognition} color="#00D4FF">
								<Typography variant="caption">
									Recognition: {(voiceRecognition * 100).toFixed(1)}%
								</Typography>
								<Box className="calibration-bar" />
							</CalibrationIndicator>
							{voiceCommand && (
					...
					{activeMode === InteractionMode.EYE_TRACKING && (
						<Box>
							...
							<CalibrationIndicator value={eyeTrackingAccuracy} color="#FF6B00">
								<Typography variant="caption">
									Accuracy: {(eyeTrackingAccuracy * 100).toFixed(1)}%
								</Typography>
								<Box className="calibration-bar" />
							</CalibrationIndicator>
							<Box
