...
<Card>
	...
	<p class="text-gray-600 mb-6">
		For each statement, choose whether you agree ("Yes") or disagree ("No").
	</p>
				...
				<fieldset class="border-t border-gray-200 pt-4">
					...
					<div class="flex items-center space-x-4 mt-2">
						<label class="flex items-center cursor-pointer">
							<input type="radio" name="q{question.id}" value="yes" class="radio radio-primary" bind:group={answers[question.id]} required />
							<span class="ml-2">Yes</span>
						</label>
						<label class="flex items-center cursor-pointer">
							<input type="radio" name="q{question.id}" value="no" class="radio" bind:group={answers[question.id]} required />
