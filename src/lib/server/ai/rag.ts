...
const MATCH_THRESHOLD = 0.75; // Ambang batas kemiripan minimum
const MATCH_COUNT = 5; // Jumlah maksimum potongan konteks yang akan diambil
...
export async function retrieveContext(queryText: string): Promise<KnowledgeChunk[]> {
	...
	try {
		...
		logger.debug({ query: queryText }, 'Membuat embedding untuk query RAG...');
		...
		if (!data || data.length === 0) {
			logger.info({ query: queryText }, 'Tidak ada konteks relevan yang ditemukan.');
			return [];
		}
		...
		logger.error({ error: e, query: queryText }, 'Terjadi error tak terduga dalam pipeline RAG.');
