...
export class NotionService {
	...
	async findOpenProjects(databaseId: string) {
		...
		try {
			...
			logger.warn('NotionService.findOpenProjects failed with filter. Fallback to unfiltered.', {
				err
			});
	...
	async getDocumentByName(
		databaseId: string,
		docName: string
	): Promise<{ pageId: string; title: string; text: string } | null> {
		const client = await this.getClient();
		...
		const query = async () => {
			try {
				...
				logger.warn(
					'NotionService.getDocumentByName failed Title filter. Fallback to manual search.',
					{ err }
				);
				const res = await client.databases.query({ database_id: databaseId, page_size: 100 });
				return {
					results: res.results.filter(
						(p: any) => this.extractTitle(p).trim().toLowerCase() === docName.trim().toLowerCase()
					)
				} as any;
			}
		};
	...
	private mapProject(page: any) {
		const title = this.extractTitle(page);
		const tags =
			this.extractMultiSelect(page, 'Tags') || this.extractMultiSelect(page, 'Kategori') || [];
