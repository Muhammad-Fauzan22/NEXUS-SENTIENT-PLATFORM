...
export const GET: RequestHandler = async ({ url }) => {
	...
	try {
		...
		const health = {
			...
			correlationId,
			services: {
				database: dbResult.status === 'fulfilled' ? dbResult.value : { status: 'unhealthy', error: dbResult.reason },
				ai: aiResult.status === 'fulfilled' ? aiResult.value : { status: 'unhealthy', error: aiResult.reason },
				system: systemResult.status === 'fulfilled' ? systemResult.value : { status: 'unhealthy', error: systemResult.reason }
			}
		};
