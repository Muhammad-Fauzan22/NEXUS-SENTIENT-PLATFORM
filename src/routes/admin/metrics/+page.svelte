<script lang="ts">
  ...
  let metrics: { knowledge_chunks?: number|null; skill_trends?: number|null; topSkills?: { skill_name: string; velocity_score: number }[]; serverTime?: string } = $state({});
...
<div class="p-6 max-w-5xl mx-auto">
  ...
  <div class="text-sm text-foreground/70">Server Time: {metrics.serverTime || '-'} <button class="ml-3 underline" onclick={loadMetrics}>Refresh</button></div>
