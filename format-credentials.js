...
async function formatCredentials() {
	...
	try {
		...
		logger.debug(
			`[Format Credentials Script] File kredensial berhasil dibaca (${credentialsBuffer.length} bytes).`
		);
		...
		if (missingFields.length > 0) {
			throw new Error(
				`Field kredensial yang diperlukan tidak ditemukan: ${missingFields.join(', ')}`
			);
		}
		...
		logger.info(
			`[Format Credentials Script] Menyimpan kredensial yang diformat ke: ${outputFilePath}`
		);
		...
		console.log('');
		console.log(
			'📋 Untuk menggunakannya sebagai environment variable (misalnya, GOOGLE_CREDENTIALS_JSON_BASE64):'
		);
		...
		console.log(
			`      [Convert]::ToBase64String([IO.File]::ReadAllBytes("${OUTPUT_FILE_NAME}")) > encoded-google-credentials.txt`
		);
		console.log(`      # Di Windows (Command Prompt):`);
		console.log(
			`      certutil -encode ${OUTPUT_FILE_NAME} encoded-google-credentials.txt && findstr /v /c:- encoded-google-credentials.txt > temp.txt && move /y temp.txt encoded-google-credentials.txt`
		);
		console.log('');
